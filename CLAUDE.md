...
### Editorial Media (Audio + Video)
- Each editorial gets two media artifacts generated via NotebookLM:
  1. **Brief anime-style video** (~1-2 min): `editorial-video-YYYY-MM-DD.mp4`, uploaded to YouTube
  2. **Brief audio overview** (~5-8 min): `editorial-podcast-YYYY-MM-DD.mp3`, uploaded to Supabase
- Generated from editorial article text + LLM-created direction documents (video + audio)
- Uses the same permanent notebook as the digest (sources cleaned between runs)
- Video uses `generate_video()` with brief format + anime visual style (NOT cinematic)
- Audio uses `generate_audio()` with brief format
- Direction docs generated by `generate_editorial_video_direction()` and `generate_editorial_audio_direction()` in `04_generate_media.py`
- Media strip embedded in editorial HTML template (2-col grid after hero, before article body)
- Also included in daily email newsletter as "Deep Dive Media" section
- Status written to `editorial-media-status.json`
- CLI: `notebooklm_media.py --editorial-file article.txt --editorial-video-direction vid.txt --editorial-audio-direction aud.txt`
